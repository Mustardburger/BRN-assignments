...
class TicTacToe:
    def __init__(self, player_symbol, computer_symbol, board_size=3):
        ...
        self.mapping = {1: self.player_symbol, 0: ".", -1: self.computer_symbol}
        self.reverse_mapping = {self.player_symbol: 1, ".": 0, self.computer_symbol: -1}
    ...
    def make_move(self, piece: str, loc: tuple):
        ...
        """
        row, col = loc
        if not self.check_legal(piece, loc):
            return False
    ...
    def check_win(self):
        ...
        # Player 1 wins
        if (
            np.any(sum_cols == self.board_size)
            or np.any(sum_rows == self.board_size)
            or self.board_size == sum_left_diag
            or self.board_size == sum_right_diag
        ):
            return 1
    ...
    def print_board(self):
        ...
        printable_board = [
            [self.mapping[self.board[i, j]] for j in range(self.board_size)]
...
class PlayTicTacToe:
    ...
    def play(self):
        ...
        while self.game.check_win() == 0 and not self.game.check_draw():
            ...
            while (
                not self.game.check_legal(self.player_symbol, (row, col))
                or confirm == "n"
            ):
                ...
                col = input("What col? ")
                confirm = input(
                    f" Place {self.player_symbol} at row={row} and col={col} [y/n]? "
                )
                ...
                if not self.game.check_legal(self.player_symbol, (row, col)):
                    print(
                        f"Invalid value. Row and col should be between 0 and {self.board_size - 1} and should not occupy already placed squares"
                    )
            ...
            self.game.make_move(
                piece=self.computer_symbol, loc=(computer_row, computer_col)
            )
